...
def inversao(args):
    id, Pc, Wc, xi, w0, ni, nt2, n_sens, f_scale_val = args
    ...
    Z0_id = np.zeros(ni, dtype=np.float32) # Vetor para armazenar Z0 direto
    ...
    for i in range(ni):
        ...
        if np.sum(validos) < 2:
            ...
            try:
                ...
                # Desempacotando a resposta do otimizador
                Z0_rec = res.x[0]
                crec = res.x[1]
                
                # Recuperando a densidade a partir da impedância
                prec = Z0_rec / crec
...
if __name__ == '__main__':
    ...
    f_scale_val = 0.25  # <--- Altere este valor para testar diferentes limites L1
    ...
    print(f"Usando {num_nucleos} núcleos. Iniciando loop de tiros com SNR = {SNR_dB} dB e f_scale = {f_scale_val}")
    ...
    with Pool(processes=num_nucleos) as pool:
        for tiro in range(1, N_tiros + 1):
            ...
            arg = []
            for id in range(Nx_idx):
                arg.append((id, P_canal[id, :, :], W_canal[id, :, :], xi, w0, ni, nt2, n_sens, f_scale_val))
            
            resultados = pool.map(inversao, arg)
            
            # Desempacotando e acumulando a Impedância
    ...
    # Salvando estatísticas em um arquivo txt com o f_scale no nome
    nome_arquivo_txt = f'estatisticas_erro_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.txt'
    with open(nome_arquivo_txt, 'w', encoding='utf-8') as f:
        f.write(f"Resultados da Inversão Sísmica - {N_tiros} tiro(s) | SNR: {SNR_dB} dB | f_scale: {f_scale_val}\n")
        f.write("-" * 65 + "\n")
    ...
    axes_v[1].set_title(f'Inversão com Interpolação - Velocidade (SNR {SNR_dB}dB | f_scale {f_scale_val})', fontweight='bold')
    ...
    fig_v.tight_layout()
    fig_v.savefig(f'erro_marmousi_velocidade_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.png', dpi=300)
    plt.close(fig_v)
    ...
    axes_r[1].set_title(f'Inversão com Interpolação - Densidade (SNR {SNR_dB}dB | f_scale {f_scale_val})', fontweight='bold')
    ...
    fig_r.tight_layout()
    fig_r.savefig(f'erro_marmousi_densidade_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.png', dpi=300)
    plt.close(fig_r)
    ...
    axes_z[1].set_title(f'Inversão Direta com Interpolação - Impedância (SNR {SNR_dB}dB | f_scale {f_scale_val})', fontweight='bold')
    ...
    fig_z.tight_layout()
    fig_z.savefig(f'erro_marmousi_impedancia_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.png', dpi=300)
    plt.close(fig_z)
