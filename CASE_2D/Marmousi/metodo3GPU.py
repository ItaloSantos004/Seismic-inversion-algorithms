...
def otimizar_parametros_gpu(Zrec, xi_gpu, xi2_gpu, w0, lim, alpha, n_iters=500):
    ...
    # Otimização em lote simultânea para todas as colunas e profundidades
    for _ in range(n_iters):
        ...
        res_weighted = res * pesos_gauss
        
        loss = torch.where(validos, res_weighted**2, torch.zeros_like(res_weighted)).sum()
...
if __name__ == '__main__':
    ...
    # ========================================================
    # LOOP DE VARREDURA DOS ALPHAS (GPU)
    # ========================================================
    lista_alphas = [0.6, 0.8, 1.0, 1.2, 1.5, 2.0]
    
    for alpha in lista_alphas:
        print("\n" + "="*50)
        print(f"Iniciando inversão na GPU com alpha = {alpha}")
        print("="*50)
        
        # Reset de sementes para garantir reprodutibilidade e igualdade de ruído
        ...
        for tiro in range(1, N_tiros + 1):
            ...
            # Inversão Layer-Peeling vetorizada
            Zrec = inversao_layer_peeling_gpu(P_canal, W_canal, ni, nt2, Z_min, Z_max)
            
            # Otimização dos parâmetros em GPU com pesos gaussianos por alpha
            vel_tiro, rho_tiro, Z0_tiro = otimizar_parametros_gpu(Zrec, xi_gpu, xi2_gpu, w0, lim, alpha, n_iters=500)
        ...
        # Interpolação para a malha original do Marmousi
        ...
        with open(nome_arquivo_txt, 'w', encoding='utf-8') as f:
            f.write(f"Resultados da Inversão GPU - alpha = {alpha} | SNR: {SNR_dB} dB | Tiros: {N_tiros}\n")
            f.write("-" * 80 + "\n")
        ...
        axes_v[1].set_title(f'Inversão GPU - Velocidade (alpha={alpha} | SNR {SNR_dB}dB)', fontweight='bold')
        ...
        axes_r[1].set_title(f'Inversão GPU - Densidade (alpha={alpha} | SNR {SNR_dB}dB)', fontweight='bold')
        ...
        axes_z[1].set_title(f'Inversão GPU - Impedância (alpha={alpha} | SNR {SNR_dB}dB)', fontweight='bold')
    ...
    print("Varredura de alphas na GPU totalmente concluída!")