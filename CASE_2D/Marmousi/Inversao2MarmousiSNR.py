...
def inversao(args):
    ...
    Z0_id = np.zeros(ni, dtype=np.float32) # NOVO: Vetor para armazenar Z0 direto
    ...
    return id, vel_id, rho_id, Z0_id # Agora retorna a impedância diretamente
...
if __name__ == '__main__':
    ...
    Z0_acumulada = np.zeros((ni, Nx_idx), dtype=np.float32) # NOVO: Acumulador de Z0
    ...
    Z0_rec_media = Z0_acumulada / N_tiros # NOVO: Média da impedância
    ...
    Zint = np.zeros((ni, nxm), dtype=np.float32) # NOVO: Matriz para Z interpolada
    ...
    for i in range(ni):
        ...
        # NOVO: Interpolando a Impedância diretamente, sem multiplicar v*rho
    ...
    fig_v.tight_layout()
    fig_v.savefig(f'erro2_marmousi_velocidade_{N_tiros}_SNR{SNR_dB}.png', dpi=300)
    plt.close(fig_v)
    ...
    fig_r.tight_layout()
    fig_r.savefig(f'erro2_marmousi_densidade_{N_tiros}_SNR{SNR_dB}.png', dpi=300)
    plt.close(fig_r)
    ...
    fig_z.tight_layout()
    fig_z.savefig(f'erro2_marmousi_impedancia_{N_tiros}_SNR{SNR_dB}.png', dpi=300)
    plt.close(fig_z)
