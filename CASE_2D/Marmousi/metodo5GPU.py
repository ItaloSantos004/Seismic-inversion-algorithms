...
def otimizar_parametros_gpu(Zrec, xi2, w0, lim, f_scale_val, n_iters=500):
    ...
    # 1. Pega a impedância do termo central (incidência normal xi = 0)
    Z0_inc_normal = Zrec[:, n_sens // 2, :].clamp(Z0_min + 1e-3, Z0_max - 1e-3)
    
    # 2. Converte para o espaço não-linear do Sigmoid
    p_init = (Z0_inc_normal - Z0_min) / (Z0_max - Z0_min)
    u_Z_init = torch.log(p_init / (1.0 - p_init))
    
    # Inicializa u_Z com o termo central em vez de zeros
...
if __name__ == '__main__':
    ...
    # Passando os dados originais direto para a GPU
    ...
    print(f"Executando no dispositivo: {device}. Iniciando loop de tiros com SNR = {SNR_dB} dB e f_scale = {f_scale_val}")

    for tiro in range(1, N_tiros + 1):
        print(f"Loop: {tiro}/{N_tiros}")
        
        # Adição de ruído vetorizada na GPU
        ...
        # Transformada de Fourier vetorizada no espaço de receptores (dim=1)
    ...
    nome_arquivo_txt = f'dados_erro_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.txt'
    with open(nome_arquivo_txt, 'w', encoding='utf-8') as f:
        f.write(f"Resultados da Inversão Sísmica GPU - {N_tiros} tiro(s) | SNR: {SNR_dB} dB | f_scale: {f_scale_val}\n")
        f.write("-" * 65 + "\n")
    ...
    fig_v.tight_layout()
    fig_v.savefig(f'erro_velocidade_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.png', dpi=300)
    plt.close(fig_v)
    ...
    fig_r.tight_layout()
    fig_r.savefig(f'erro_densidade_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.png', dpi=300)
    plt.close(fig_r)
    ...
    fig_z.tight_layout()
    fig_z.savefig(f'erro_impedancia_{N_tiros}_SNR{SNR_dB}_fscale{f_scale_val}.png', dpi=300)
    plt.close(fig_z)
