...
print(f"-> Simulando Grade: {Nx}x{Nz} pixels.")
print("-> Resolvendo PML Definitivo: Staggered ADE-PML (Grade Intercalada).")
...
# =========================================================================
# 3. PERFIL DE AMORTECIMENTO (Cálculo Preciso Sub-Pixel)
# =========================================================================
L_pml = 100
R = 0.0001 # Aumentei a força de absorção (10^-4)
V_max = np.max(Vp)

# Equação de perfil grau 3
n_pml = 3.0
...
# Vetores nos NÓS (onde a onda U vive)
sx = np.zeros(Nx, dtype=np.float32)
sz = np.zeros(Nz, dtype=np.float32)

# Vetores nas FACES (onde a memória Qx e Qz vivem - Grade Intercalada)
...
def calcular_staggered_pml(U, Qx, Qz, sx, sx_half, sz, sz_half, Vp, dx, dz, dt, p1, p2, p3, q1, q2, Nz, Nx):
    ...
    # PASSO 1: Atualizar Qx (A Memória que vive no Eixo X entre os pixels)
    ...
    # PASSO 2: Atualizar Qz (A Memória que vive no Eixo Z entre os pixels)
    ...
    # PASSO 3: Atualizar a Onda Principal U
    for i in prange(1, Nz - 1):
        for j in range(1, Nx - 1):
            
            # Trazemos as derivadas da memória de volta para o centro do pixel com perfeição
...
print("===================================================")
print("INICIANDO SIMULAÇÃO À PROVA DE REFLEXÕES...")
print("===================================================")
...
for n in range(1, tempo + 1):
    ...
    if n % 20 == 0:
        if n == 20:
            ...
            title_text = ax.set_title(f'Staggered ADE-PML (Tempo = {t_atual:.3f} s)', fontweight='bold')
            fig.tight_layout()
        else:
            im.set_data(U[:, :, p3])
            title_text.set_text(f'Staggered ADE-PML (Tempo = {t_atual:.3f} s)')
        
        fig.canvas.draw()
        imagem_matriz = np.asarray(fig.canvas.buffer_rgba()).copy()
        writer.append_data(imagem_matriz)
        print(f"-> Passo {n} / {tempo} concluído...")
...
writer.close()
plt.close()
print(f"\nTeste finalizado! Verifique o arquivo '{arq_gif}'.")