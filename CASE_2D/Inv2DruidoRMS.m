...
%injeção de ruido
nivel_ruido = 0.0; %nivel de ruido

rms_P = sqrt(mean(Psensor(:).^2));
rms_W = sqrt(mean(Wsensor(:).^2));

piso_P = nivel_ruido * rms_P;
piso_W = nivel_ruido * rms_W;

Psensor = Psensor + piso_P * randn(size(Psensor));
Wsensor = Wsensor + piso_W * randn(size(Wsensor));
...
if sum(validos) < 2 
    ...
    fun_min = @(p) sum( abs( Z_medido - (p(1)*w) ./ sqrt( (w/p(2)).^2 - xi3 ) ).^2 ) ...
                + 1e12 * (p(1) <= 0) + 1e12 * (p(2) <= 0) + 1e12 * ((w/p(2))^2 <= max(xi3)); 
...
%salvando a imagem
nome = sprintf('Resultado_RuidoRMS_%.3f.png', nivel_ruido);
