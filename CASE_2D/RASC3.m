...
z_sup_reg = 2 : interface_meio - 1; %superior
z_int     = interface_meio; %interface
z_inf_reg = interface_meio + 1 : Nz - 1; %inferior
i_reg     = 2 : Nx - 1; %eixo x
...
%interface
K_media = (p1*c1^2 + p2*c2^2) / 2;
p_media = (p1 + p2) / 2;
c_int_eff = sqrt(K_media / p_media);
S_int = c_int_eff * dt / h;
c2_int = [1, -(S_int - 1)/(S_int + 1); (S_int - 1)/(S_int + 1), -1];
c3_int = 1;
    ...
    if M > 0
        ...
        for idxi = 1:M+1
            for idxj = 1:M+1
                ...
                U(1,  z_inf_reg, p+1) = U(1,  z_inf_reg, p+1) - coef_lat_inf * U(idxj, z_inf_reg, tn);
                U(Nx, z_inf_reg, p+1) = U(Nx, z_inf_reg, p+1) - coef_lat_inf * U(Nx - idxj + 1, z_inf_reg, tn);
            end
        end
    end
